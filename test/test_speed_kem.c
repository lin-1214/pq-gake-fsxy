...
static OQS_STATUS kem_speed_wrapper(const char *method_name, uint64_t duration, bool printInfo) {
	...
	TIME_OPERATION_SECONDS(
		...
		OQS_KEM_keypair(kem, public_key, secret_key), "keygen", duration
	)
	TIME_OPERATION_SECONDS(OQS_KEM_encaps(kem, ciphertext, shared_secret_e, public_key, coins), "encaps", duration)
