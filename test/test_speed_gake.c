    ...
    if(OQS_KEM_alg_is_enabled(OQS_KEM_alg_identifier(alg))){
      ...
      TIME_OPERATION_ITERATIONS(
        for (int j = 0; j < NUM_PARTIES; j++) {
          ...
          int res = check_xs(kem, pointer_to_parties, j, NUM_PARTIES, kem->length_shared_secret); // Check Xi
          int result = check_commitments(pointer_to_parties, j, NUM_PARTIES, kem->length_shared_secret); // Check commitments

          if (res == 0) {
          } else {
            pointer_to_parties[j].acc = 0;
            pointer_to_parties[j].term = 1;
            return 1;
          }
