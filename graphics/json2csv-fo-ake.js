...
function handlePipedContent() {
    ...
    stdin.on('end', function() {
      ...
      if (type === "fo-gake") header += `,N`
      stdout.write(header + "\n");
      stdout.write(
        array_chunks(
          JSON.parse(data).slice(1), type !== "fo-gake" ? 4 : 5).map(x => {
            return x.slice().map(
              y => ({alg, ...y})
            )
          }
         ...
         .map(x => {
           str = `${type},${alg},${x.Operation},${x.Iterations},${x["Total time (s)"]},${x["Time (us): mean"]},${x["pop. stdev"]},${x["CPU cycles: mean"]}`
           if (type === "fo-gake") str += `,${N}`
           return str
         })
         .join("\n") + "\n"
      )
